    ...
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string categorySlug="", int p = 1)
        {
            ...
            if (categorySlug == "")
            {
                ...
                return View(
                    await _context.Products.OrderByDescending(p => p.Id).Skip((p -1) * pageSize).Take(pageSize).ToListAsync()
                    );
            }
            ...
            return View(
                await productsByCategory.OrderByDescending(p => p.Id).Skip((p - 1) * pageSize).Take(pageSize).ToListAsync()
                );
        }
    }
}
